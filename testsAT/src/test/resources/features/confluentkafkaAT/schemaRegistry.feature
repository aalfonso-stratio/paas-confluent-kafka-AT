...
Feature: Producing/consuming data using schema registry
  ...
  Scenario: Install schema-registry-sec
    ...
    And I run 'dcos marathon task list schema-registry-sec | awk '{print $5}' | grep schema-registry-sec' in the ssh connection and save the value in environment variable 'marathonTaskId'
  ...
  Scenario: Launch producer
    ...
    Given I run 'dcos task | grep broker-0 | awk '{print $2}'' in the ssh connection and save the value in environment variable 'broker0Host'
    # Log to broker-0
    And I open a ssh connection to '!{broker0Host}' with user '${DCOS_USER}' and password '${DCOS_PASSWORD}' using pem file '${PEM_FILE}'
    # Obtain sandbox directory
    And I run 'ps -ef | grep java | grep kafka_confluent-oss  | awk '{print $8;}' | awk -F'/jre' '{print $1}'' in the ssh connection and save the value in environment variable 'sandboxDirectory'
    ...
    And I authenticate to DCOS cluster '${DCOS_IP}' with email '${DCOS_USER}', user '${REMOTE_USER}' and password '${REMOTE_PASSWORD}' using pem file '${PEM_FILE}'
    And I securely send requests to '${DCOS_IP}:443'
    And I send a 'GET' request to '/mesos_dns/v1/services/_broker-0._tcp.confluent-kafka-sec.mesos'
    And I save element '$.[0].port' in environment variable 'broker0Port'
    # Produce messages
  ...
  Scenario: Launch consumer
    When I run './kafka-avro-console-consumer.sh --topic topic2 --bootstrap-server broker-0.confluent-kafka-sec.mesos:9179  --consumer.config=../config/consumer.properties'
